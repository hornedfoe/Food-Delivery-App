                    ...
                    <div className="collapse navbar-collapse" id="navbarSupportedContent">
                        ...
                        {(!localStorage.getItem("token")) ?
                            <form className="d-flex">
                                <Link className="btn bg-white mx-1 " to="/login" style = {{color: "violet"}}>Login</Link>
                                <Link className="btn bg-white mx-1" to="/signup" style = {{color: "violet"}}>Signup</Link>
                            </form> :
                            <div>

                                <div className="btn bg-white mx-2 " onClick={loadCart} style = {{color: "violet"}}>
                                    Cart
                                </div>

                                {cartView ? <Modal onClose={() => setCartView(false)}><Cart></Cart></Modal> : ""}

                                <button onClick={handleLogout} className="btn bg-white" style = {{color: "violet"}}>Logout</button></div>}
                    </div>
                </div>
            </nav>
        </div>
